    ...
    try:
        with st.spinner("Testing API connection..."):
            if not test_sarvam_api(api_key, base_url):
                st.error(
                    "Failed to connect to Sarvam API. Check your API key and base URL."
                )
                return False
...
def main():
    ...
    st.markdown(
        "Upload PDF documents and ask questions about their content using Sarvam AI."
    )
    ...
    with col2:
        ...
        if st.session_state.processing_complete:
            ...
            st.info(
                "Upload PDFs and enter your API key to start. Adjust parameters for longer or shorter answers."
            )
